  ...
  ipcMain.handle('browser:open', async (_event, url?: string) => {
    ...
    win.webContents.on('did-finish-load', () => {
      win.webContents.executeJavaScript(`
        (function() {
          if (document.getElementById('ds-agent-nav-bar')) return;
          const bar = document.createElement('div');
          bar.id = 'ds-agent-nav-bar';
          bar.innerHTML = '<div style="display:flex;gap:6px;padding:6px 10px;background:#1e1e1e;border-bottom:1px solid #333;align-items:center">' +
            ...
            '<input id="ds-agent-url" value="' + location.href + '" style="flex:1;background:#2d2d2d;border:1px solid #444;color:#ccc;padding:4px 8px;border-radius:4px;font-size:12px;outline:none" onkeydown="if(event.key===\\'Enter\\')location.href=this.value">' +
            '</div>';
